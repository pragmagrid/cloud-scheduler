			...
			{foreach from=$BoundDates item=date}
				...
				{foreach from=$slots item=slot name=slot_loop}
					...
					{capture assign="slotContent"}
						{displaySlot Slot=$slot Href="$href" AccessAllowed=$resource->CanAccess SlotRef=$slotRef spantype='row' ResourceId=$resource->Id ResourceAttributes=$resourceAttributes}
					{/capture}
