...
class PreReservationFactory implements IPreReservationFactory
{
	...
	private function GetAddUpdateRuleProcessor(UserSession $userSession)
	{
		...
		// SSM: Admins should not be exempt from CustomAttributeValidationRule
		$ruleProcessor->AddRule(new CustomAttributeValidationRule(new ResourceReservationAvailability($this->reservationRepository), new AttributeService(new AttributeRepository())), $userSession, $this->userRepository);
